...
_SAMPLE_RATE = 48000
# _FPS is the rounded frame rate used only for buffer sizing and keyframe
# interval.  The true DS rate (DS_FRAMERATE ≈ 59.8261) is what we report
# to ffmpeg so audio/video timestamps stay aligned.
_FPS = 60
...
class HLSStreamer:
    ...
    def _write_frames(self) -> None:
        ...
        try:
            ...
            with open(self._video_fifo, "wb") as vf:
                ...
                # Write matching silence for the primer frame — one DS
                # frame's worth at 48 kHz stereo (~802 samples × 4 bytes).
                # Exact count doesn't matter for sync: ffmpeg timestamps
                # audio by cumulative sample count, so the primer just
                # becomes the first few ms of the stream.
                af.write(b"\x00" * 3200)
                af.flush()
                ...
                while self._running:
                    ...
                    if self._rt_origin is None:
                        ...
                        self._rt_frames += 1
                        content_secs = self._rt_frames / DS_FRAMERATE
    ...
    def _on_cycle(self) -> None:
        ...
        # Drain whatever audio the SPU produced this frame.  The DS runs
        # at ~59.8261 Hz and the SPU outputs 48 kHz, so each frame yields
        # ~802.3 stereo samples on average.  Because we declare the true
        # DS frame rate to ffmpeg for the video input, emitting the exact
        # number of samples produced each cycle keeps audio and video in
        # sync indefinitely — ffmpeg timestamps audio by cumulative
        # sample count, and 48000 samples/sec naturally align with
        # 59.8261 frames/sec of video when both rates are truthful.
        ...
        if self._blocking:
            try:
                self._frame_queue.put((raw_rgb, audio_data), timeout=5.0)
            except queue.Full:
                self._drop_count += 1
                logger.warning(
            ...
            try:
                self._frame_queue.put_nowait((raw_rgb, audio_data))
            except queue.Full:
                self._drop_count += 1
